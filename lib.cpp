...
// System::String^ → char* 型変換
// 注意：本関数内部で *dst の領域確保を行うため、使用後はdst領域を必ず free() すること。
// 引数：
// src[in]  変換したいStringクラスのポインタ
// dst[out] ※ポインタだけ渡す。
// メモリ確保したポインタを渡しちゃうと書き換えちゃうので注意
int String2Charp(System::String^ src, char **dst)
{
	...
	setlocale( LC_ALL, "Japanese" );	// 全角文字変換対応

	// System::String(src)→ワイド文字列(wch)
	pin_ptr<const wchar_t> wch = PtrToStringChars(src);		// PtrToStringChars with vcclr.h

	// ワイド文字列(wch)→マルチバイト文字列(*dst)
	len = (src->Length + 1) * 2;
	*dst = (char *)malloc( len );
